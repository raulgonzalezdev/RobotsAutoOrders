...
void TrailingStopByValue(const double value_sl, const long magic = -1, const int trailing_step_pt = 0, const int trailing_start_pt = 0)
{
    ...
    for (int i = total - 1; i >= 0; i--)
    {
        ...
        // Nueva lógica para FOREX
        if ((Symbol() == "BTCUSD" || Symbol() == "BITCOIN") &&
            (TimeCurrent() - PositionGetInteger(POSITION_TIME) > PeriodSeconds(CryptoTimeframe)) &&
            (PositionGetDouble(POSITION_PROFIT) < -2 * ProfitEstimated))
        {
            // Lógica existente para ajustar SL
            double adjusted_value_sl = value_sl;
            if (pos_type == POSITION_TYPE_SELL)
            {
                adjusted_value_sl = MathMax(value_sl, tick.ask + StopLevel(98) * Point());
                pos_tp = CalcularTakeProfit(tick.ask, pos_type);
            }
            else if (pos_type == POSITION_TYPE_BUY)
            {
                adjusted_value_sl = MathMin(value_sl, tick.bid - StopLevel(98) * Point());
                    ...
                    int trailing_step_pt, int trailing_start_pt, MqlTick &tick)
{
    ...
    double stop_level = StopLevel(98) * Point();
    int pos_profit_pt = 0;
...
bool MarketOpenHours(string sym)
{
    ...
    {                                                     // is today in monday to friday?
        ...
        switch (today)
        { // check for different trading sessions
        case 1:
