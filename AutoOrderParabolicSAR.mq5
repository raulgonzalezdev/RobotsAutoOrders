...
// Definición del ENUM para los tipos de mercado
enum ENUM_MERCADO
{
    CRYPTO,
    FOREX,
...
// Definición de ENUMs para valores recomendados
enum ENUM_RISK_PERCENT
{
    RISK_CRYPTO = 50,  // Representa 0.5
    RISK_FOREX = 100,  // Representa 1.0
    RISK_INDICES = 100,
    RISK_COMMODITIES = 100,
    RISK_STOCKS = 100
};

enum ENUM_PROFIT_ESTIMATED
{
    PROFIT_CRYPTO = 5,
    PROFIT_FOREX = 2,
    PROFIT_INDICES = 5,
    PROFIT_COMMODITIES = 5,
    PROFIT_STOCKS = 2
};

enum ENUM_TAKE_PROFIT_PERCENT
{
    TP_CRYPTO = 100,  // Representa 1.0
    TP_FOREX = 50,    // Representa 0.5
    TP_INDICES = 100,
    TP_COMMODITIES = 100,
    TP_STOCKS = 50
};

// Parámetro de entrada para seleccionar el mercado
input ENUM_MERCADO Mercado = CRYPTO;

// Parámetros de entrada con valores recomendados
input ENUM_RISK_PERCENT RiskPercentEnum = RISK_CRYPTO;
input double RiskPercent = 0.5; // Permite al usuario ingresar cualquier valor

input ENUM_PROFIT_ESTIMATED ProfitEstimatedEnum = PROFIT_CRYPTO;
input double ProfitEstimated = 5; // Permite al usuario ingresar cualquier valor

input ENUM_TAKE_PROFIT_PERCENT TakeProfitPercentEnum = TP_CRYPTO;
input double TakeProfitPercent = 1.0; // Permite al usuario ingresar cualquier valor

// Otros parámetros de entrada
input bool FixedVolume = true;
input double ParabolicStep = 0.01;
input double ParabolicMax = 0.1;
input double ADXThreshold = 11.0;
input int ADXPeriod = 7;
input int CryptoFastPeriod = 10;
input int CryptoSlowPeriod = 30;
input ENUM_TIMEFRAMES CryptoTimeframe = PERIOD_M5;
...
input string TipoOrdenManual = "SELL"; //



// Variables internas
double RiskPercentInternal;
double ProfitEstimatedInternal;
double TakeProfitPercentInternal;
bool FixedVolumeInternal;
double ParabolicStepInternal;
double ParabolicMaxInternal;
double ADXThresholdInternal;
int ADXPeriodInternal;
int FastPeriodInternal;
int SlowPeriodInternal;
ENUM_TIMEFRAMES TimeframeInternal;

//+------------------------------------------------------------------+
//| Función para inicializar parámetros según el mercado             |
//+------------------------------------------------------------------+
void InicializarParametrosPorMercado(ENUM_MERCADO mercado)
{
    switch (mercado)
    {
        case CRYPTO:
            RiskPercentInternal = RiskPercentEnum == RISK_CRYPTO ? RiskPercentEnum / 100.0 : RiskPercent;
            ProfitEstimatedInternal = ProfitEstimatedEnum == PROFIT_CRYPTO ? ProfitEstimatedEnum : ProfitEstimated;
            TakeProfitPercentInternal = TakeProfitPercentEnum == TP_CRYPTO ? TakeProfitPercentEnum / 100.0 : TakeProfitPercent;
            FixedVolumeInternal = FixedVolume;
            ParabolicStepInternal = ParabolicStep;
            ParabolicMaxInternal = ParabolicMax;
            ADXThresholdInternal = ADXThreshold;
            ADXPeriodInternal = ADXPeriod;
            FastPeriodInternal = CryptoFastPeriod;
            SlowPeriodInternal = CryptoSlowPeriod;
            TimeframeInternal = CryptoTimeframe;
            break;

        case FOREX:
            RiskPercentInternal = RiskPercentEnum == RISK_FOREX ? RiskPercentEnum / 100.0 : RiskPercent;
            ProfitEstimatedInternal = ProfitEstimatedEnum == PROFIT_FOREX ? ProfitEstimatedEnum : ProfitEstimated;
            TakeProfitPercentInternal = TakeProfitPercentEnum == TP_FOREX ? TakeProfitPercentEnum / 100.0 : TakeProfitPercent;
            FixedVolumeInternal = false; // Valor específico para FOREX
            ParabolicStepInternal = 0.02;
            ParabolicMaxInternal = 0.2;
            ADXThresholdInternal = 20.0;
            ADXPeriodInternal = 14;
            FastPeriodInternal = 20;
            SlowPeriodInternal = 50;
            TimeframeInternal = PERIOD_H1;
            break;

        case INDICES:
            RiskPercentInternal = RiskPercentEnum == RISK_INDICES ? RiskPercentEnum / 100.0 : RiskPercent;
            ProfitEstimatedInternal = ProfitEstimatedEnum == PROFIT_INDICES ? ProfitEstimatedEnum : ProfitEstimated;
            TakeProfitPercentInternal = TakeProfitPercentEnum == TP_INDICES ? TakeProfitPercentEnum / 100.0 : TakeProfitPercent;
            ADXThresholdInternal = 25.0;
            TimeframeInternal = PERIOD_M15;
            break;

        case COMMODITIES:
            RiskPercentInternal = RiskPercentEnum == RISK_COMMODITIES ? RiskPercentEnum / 100.0 : RiskPercent;
            ProfitEstimatedInternal = ProfitEstimatedEnum == PROFIT_COMMODITIES ? ProfitEstimatedEnum : ProfitEstimated;
            TakeProfitPercentInternal = TakeProfitPercentEnum == TP_COMMODITIES ? TakeProfitPercentEnum / 100.0 : TakeProfitPercent;
            TimeframeInternal = PERIOD_H1;
            break;

        case STOCKS:
            RiskPercentInternal = RiskPercentEnum == RISK_STOCKS ? RiskPercentEnum / 100.0 : RiskPercent;
            ProfitEstimatedInternal = ProfitEstimatedEnum == PROFIT_STOCKS ? ProfitEstimatedEnum : ProfitEstimated;
            TakeProfitPercentInternal = TakeProfitPercentEnum == TP_STOCKS ? TakeProfitPercentEnum / 100.0 : TakeProfitPercent;
            TimeframeInternal = PERIOD_M15;
            break;
    }
}
...
double Lots = RiskPercentInternal;
int OtherFastPeriod = FastPeriodInternal;
int OtherSlowPeriod = SlowPeriodInternal;
ENUM_TIMEFRAMES OtherTimeframe = TimeframeInternal;
...
int OnInit()
{
    // Llamar a la función para inicializar parámetros según el mercado
    InicializarParametrosPorMercado(Mercado);
...
void TrailingStopByValue(const double value_sl, const long magic=-1, const int trailing_step_pt=0, const int trailing_start_pt=0)
{
    ...
    for(int i = total - 1; i >= 0; i--)
    {
        ...
        {
            adjusted_value_sl = MathMax(value_sl, tick.ask + StopLevel(8) * Point());
            pos_tp = CalcularTakeProfit(tick.ask, pos_type);
        }
        else if(pos_type == POSITION_TYPE_BUY)
        {
            adjusted_value_sl = MathMin(value_sl, tick.bid - StopLevel(8) * Point());
...
double CalcularStopLossCritico(MqlTick &tick, ENUM_POSITION_TYPE posType)
{
    ...
    double stop_level = StopLevel(8) * Point();
