...
//--- Input parameters
input int InpAverBodyPeriod=6;      // period for calculating average candlestick size
input int InpMAPeriod      =3;      // Trend MA period
input int InpPeriodRSI     =10;     // RSI period
input ENUM_APPLIED_PRICE  InpPrice=PRICE_CLOSE;    // price type

//--- trade parameters
input uint InpDuration=3;           // position holding time in bars (shortened for M1/M5)
input uint InpSL      =100;         // Stop Loss in points (adjusted for lower timeframes)
input uint InpTP      =100;         // Take Profit in points (adjusted for lower timeframes)
